...
namespace DataToolChain.Ui.DbStringer
{
    public class ParseHours
    {
        ...
        private static readonly Regex HoursRegex = new Regex(@"(?<hour1>[0-9]{1,2}):?(?<minute1>[0-9]{2})?\s*(?<tod1>am|pm)?\s*-\s*(?<hour2>[0-9]{1,2}?):?(?<minute2>[0-9]{2})?\s*(?<tod2>am|pm)?($|\W)", RegexOptions.Compiled | RegexOptions.IgnoreCase);
